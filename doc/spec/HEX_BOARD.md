# Hex-Based Game Board Implementation Plan
...
Create [`src/components/HexTile.tsx`](../../src/components/HexTile.tsx) rendering SVG `<polygon>`:
...
**Create [`src/lib/hexMath.ts`](../../src/lib/hexMath.ts) with:**
...
**Create [`src/components/HexGrid.tsx`](../../src/components/HexGrid.tsx):**
...
- Filter hexes via `getValidHexesForInfrastructure` from [`src/lib/boardValidation.ts`](../../src/lib/boardValidation.ts)
...
- **water_ice**: H2O Extractor only
- **helium_3**: Helium-3 Extractor only
- **solar_energy**: Solar Array only
- **habitat_zone**: Habitat only
- **communications_hub**: Non-buildable (narrative only)
- **launch_hub**: Non-buildable (narrative only)
...
- Stroke width: 2px
- Default stroke: `#666`
- Full capacity stroke: `#ff0000` (red)
- Background gradient: `linear-gradient(180deg, #1a1a1a, #0a0a0a)`
...
CREATE INDEX idx_board_hexes_game ON board_hexes(game_id, hex_id);
CREATE INDEX idx_player_infra_location ON player_infrastructure(game_id, location);
```
...
ALTER TABLE game_state 
  ADD COLUMN board_initialized BOOLEAN DEFAULT false,
  ADD COLUMN board_type TEXT;
```

### `player_infrastructure` Update

```sql
ALTER TABLE player_infrastructure
  ALTER COLUMN location SET NOT NULL,
  ADD CONSTRAINT fk_location_game 
    FOREIGN KEY (location, game_id) 
    REFERENCES board_hexes(hex_id, game_id);
```

## Key Design Decisions

1. **Board scoping**: Each game instance has its own board_hexes rows via composite PRIMARY KEY (hex_id, game_id)
2. **Capacity rules**: Global setting of 3 infrastructure per hex, enforced cross-player
3. **Resource mapping**: Strict 1-to-1 mapping between resource types and infrastructure types (no multi-resource hexes)
4. **Click-to-build**: Players select hexes directly on board visualization rather than text input
5. **Visual feedback**: Hex borders turn red when at capacity, color-coded by section
6. **Template system**: Board configurations defined in TypeScript files, loaded into database via RPC
7. **Starter placement**: Players must select valid hex location during player creation (after board initialization)

## Implementation Notes

- All board-related code must maintain parity between real Supabase and mock implementations
- Hex coordinate validation uses axial coordinate system with constraint `q + r + s === 0`
- Infrastructure icons grouped by type on hex tiles to prevent visual clutter
- Non-buildable hexes (communications/launch hubs) shown for narrative flavor with distinct visual styling
- Board must be initialized before players can join (enforced at UI and RPC level)
