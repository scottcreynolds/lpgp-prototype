...
- **Modified `player_infrastructure` table**:
  ...
  - Added `is_active BOOLEAN` field to replace separate `is_powered` and `is_crewed` flags
...
- **BuildInfrastructureModal**:
  - Select infrastructure type from non-starter options
  - Choose owner (can build for other players)
  - Enter location
  - Real-time EV validation
  - Cost preview
  - Only available during Operations phase
...
#### Capacity System
- Dynamic calculation of available power and crew
